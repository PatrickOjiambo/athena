...
import { getCompletion } from "@/agent/framework/access";
import type { ModelOutput } from "@/agent/model";
...
export default function MessageInput() {
  const { addMessage, setLoading, isLoading, messages } = useTanteiState();
          ...
          return (
            <div className="w-full p-2 rounded-md flex flex-row items-center gap-x-2">
              ...
              <button
                ...
                disabled={isLoading}
                className={`flex items-center gap-2 px-3 py-2 rounded-full transition-colors duration-200 border border-[var(--border-color)] ${isLoading
