...
import { type Goal } from "./orchestrator";
...
export class Oracle<QuerySchema = any, ResultData = any> {
  ...
  private Reader: (
    args: QuerySchema,
  ) => Effect.Effect<ResultData | null, unknown, never>;
  Name: string;
  Description: string;
  ...
  Read(args: Goal) {
    ...
    return Effect.tryPromise({
      ...
      Effect.andThen((data) => (data ? reader(data) : Effect.succeed(null))),
    );
  }
  ...
  static define<QuerySchema = any, ResultData = any>(args: {
    ...
    return new Oracle<QuerySchema, ResultData | null>(
      args.name,
      args.description,
      args.responseSchema,
      args.querySchema,
