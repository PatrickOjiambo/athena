...
import { Oracle } from "./foracle";
import { TASK_RESPONSE_FORMATTER_SYSTEM_PROMPT } from "./SYSTEM_PROMPTS";
import { createPrompt, sendToLLM } from "@/agent/utils/prompts";
import zodToJsonSchema from "zod-to-json-schema";
import { z } from "zod";
import { type UIBLOCK } from "./actions";

type onCompletionFn =
  | undefined
  | ((result: Array<UIBLOCK> | null, error?: any) => void);

export const goalCompleteSchema = zodToJsonSchema(
  z.object({
...
export class Task {
  ...
  reporter: (
    status: "pending" | "completed" | "failed" | "reformat-failure",
    executor: Oracle | Action,
    error?: any,
  ) => void;

  static __type: "Task" = "Task";

  static async reformat(
    executor: Action,
    prevResult: any,
    context: Array<ContextItem> | undefined = undefined,
  ) {
  ...
  static async next(
    ...
    const prompt = createPrompt(
      {
        ...
        content: `
            ...
            IF YOUR RESPONSE INCLUDES SOMETHING LIKE YOU NEED TO SWAP A TOKEN TO ANOTHER, I NEED TO PROVIDE A UI BLOCK FOR THE USER TO CONFIRM THE ACTION
            Current task: 
            ${goal.task}
      ...
      },
      {
        ...
        parameters: zodToJsonSchema(
          z.object({
            action: z.enum(
              library.Actions.map((a) => a.Name).concat([
                "CHOOSE_UI_BLOCK",
              ]) as [string],
            ),
            input: z.string(),
    ...
    if (result && result?.trim()?.startsWith("{")) {
      ...
      if (!newExecutor)
        throw new Error(`Unable to find executor for action: ${action}`);
...
export class Orchestrater {
  ...
  private reporter(
    ...
    // temporary fix for the linting issue - Anthony
    void this.data_sources;
  ...
  private async buildContext(goal: Goal) {
    ...
    const contextData = await Promise.all(
      oracles.map(async (oracle_name) => {
        const oracle = this.library.GetOracle(oracle_name);

        if (!oracle) return null;

        const either = await Effect.runPromise(
          Effect.either(oracle.Read(goal)),
        );
    ...
    const CONTEXT_SUMMARY = contextData
      .map((context) => {
        if (!context) return "";
