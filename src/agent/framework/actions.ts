...
export const CHOOSE_UI_BLOCK_ACTION = Action.define<
  ParseInputSchema,
  UIBLOCK[]
>({
  ...
  action(args, goal, context) {
    ...
    console.log("Prime Directive::", primeDirective);
    ...
    return Effect.tryPromise({
      try: async () => {
        ...
        const chosenBlocks = UI_BLOCKS.filter((block) =>
          asJSON.blocks.includes(block.name),
        );

        const serializedUIData = await Promise.all(
          chosenBlocks.map(async (block) => {
        ...
        const filteredUIBlocks = serializedUIData.filter(
          (block) => block !== null,
        ) as UIBLOCK[];
